...
function html5Setup(lyr){
	...
	try{
		if(lyr.ele && !(lyr.ele.id.indexOf("MapArea")>-1) && lyr.ele.firstChild && lyr.ele.firstChild.firstChild.nodeName == "IMG")
		{		
...
function setTransForEffect(lyr)
{
	if(!isEffectPartOfTrans(lyr.tNum))
	{
		...
		lyr.eV = 15;
		lyr.eTran = 1;
		lyr.eT = setInterval( function(){transEffect(lyr);},20)
	}
}
...
function ObjLayerDoTransNS(tOut,tNum,dur,fn,eff){
	...
	else
	{
		...
		if(this.ele.id.indexOf("page")> -1)//Only for plates
		{
			...
			this.TransStyObj = document.getElementById("transPageDiv").style;
		}
...
function showIt(){
	...
	this.clipTo(this.oT,this.oR,this.oB,this.oL);
	this.show();
	if(document.getElementById("transPageDiv"))
		document.body.removeChild(document.getElementById("transPageDiv"));
